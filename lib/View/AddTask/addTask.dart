...
class AddTask extends StatefulWidget {
  ...
  const AddTask({this.event});
  @override
  _AddTaskState createState() => _AddTaskState();
}

class _AddTaskState extends State<AddTask> with ValidationMixin {
  ...
  void saveTask() async {
    try {
      if (addNewTask) {
        await dbService.saveTask(EventModel(
            title: _title.text,
            description: _description.text,
            eventDate: _eventDate,
            time: _time));
      } else {
        await dbService.updateTask(EventModel(
            id: widget.event.id,
            title: _title.text,
            description: _description.text,
            eventDate: _eventDate,
  ...
  void deleteTask() async {
    try {
      await dbService.deleteobj(widget.event.id);
      setState(() {
        processing = false;
      });
      await _goBack();
